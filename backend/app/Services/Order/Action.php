...
class Action
{
    ...
    public function sync(int $orderId, bool $force = false): void
    {
        ...
        if ($force) {
            ...
            // 这些证书状态可以强制更新 但状态不能改变, cancelling 可以改变
    ...
    public function revalidate(int $orderId): void
    {
        ...
        $cert = $order->latestCert;
        $cert->status != 'processing' && $this->error('证书状态只有是待验证才能重新验证');
    ...
    public function removeMdcDomain(int $orderId): void
    {
        ...
        $order = FindUtil::Order($orderId);
        $cert = $order->latestCert;
        $cert->status != 'processing' && $this->error('证书状态只有是待验证才能删除未验证域名');

        $this->api->removeMdcDomain($orderId);
