    ...
    protected function addCertToZip(
        ...
        if (($type == 'all' || $type == 'iis' || $type == 'tomcat') && $privateKey && $keyMatched) {
            ...
            if (($type == 'all' || $type == 'tomcat') && $this->checkJdk()) {
                $jks = 'temp.jks';
                $cmd = "keytool -importkeystore -srckeystore $pfx -srcstoretype PKCS12 -srcstorepass $password -deststoretype jks -deststorepass $password -destkeystore $jks";
        ...
        if ($type == 'all' && $keyMatched) {
            ...
            if (isset($keyDetails['type']) && ($keyDetails['type'] === OPENSSL_KEYTYPE_RSA)) {
                ...
                fclose($keyFile);
                $rsaKey = $certName.'-rsa.key';

                // 首先尝试使用 -traditional 参数
                $cmd = "openssl pkcs8 -in $key -out $rsaKey -nocrypt -traditional";
                @exec("$cmd 2>&1", $output, $returnCode);

                // 如果 -traditional 参数失败，使用 RSA 命令转换
                if ($returnCode !== 0) {
                    $cmd = "openssl rsa -in $key -out $rsaKey -traditional";
