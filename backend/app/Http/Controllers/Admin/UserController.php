...
class UserController extends BaseController
{
    ...
    public function createUser(CreateUserRequest $request): void
    {
        ...
        try {
            ...
            $mail->isSMTP();
            $mail->isHTML();
            $mail->setFrom(get_system_setting('smtp', 'sender_mail'), get_system_setting('site', 'name', 'SSL证书管理系统'));
            $mail->addAddress($user->email);
            $mail->setSubject('品众网络用户注册通知');
            $mail->Body = '您在 pzo.cn 注册的用户名： '.$user->username.' ，密码： '.$password.' ，请妥善保管。';
            $mail->send();
        } catch (Exception) {
        }
